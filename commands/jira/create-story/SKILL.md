...
**Important:** 
- Use `--parent` flag with the parent Epic key (e.g., "SRVKP-11633")
- Do NOT use custom fields like `customfield_12311140` for epic linking
- Stories can be created without a parent (omit `--parent` flag)
- Use `$(cat <<'EOF' ... EOF)` for inline descriptions (no temp files)
...
**Implementation:**
1. Pre-validate that SRVKP-9000 exists and is an Epic
...
in [`../_common.md`](../_common.md). The preview label for this type is `PREVIEW: SRVKP User Story`.

1. ✅ **ENFORCE SRVKP-only** - reject other projects
2. 🔍 **PHASE 0A: Duplicate Detection** - search for similar stories (3+ keyword match)
3. 🔗 **PHASE 0B: Parent Epic Discovery** - search for related Epics (offer 'none' option)
4. 🔍 **Suggest component** from summary keywords
5. 💬 Interactively collect user story (Who/What/Why)
6. 💬 Interactively collect acceptance criteria
7. 💬 Optionally collect additional context
8. 📝 Format description with SRVKP template
9. 🔒 Scan for sensitive data
10. ✅ Validate story quality and completeness
11. 📋 **SHOW COMPLETE PREVIEW** (including parent epic if selected)
12. ❓ **GET USER CONFIRMATION (yes/no)**
13. ✅ **Create the story** (only if confirmed) — MCP first; same payload via `acli` on fallback (use `--parent` when linking to an epic)
14. 📤 Return issue key and URL
